...
import './SvgEmoticonBuilder.sol';

library SvgEmoticonComposer {

  struct SvgDescriptor {
    string seed;
    string svgVal;
    SvgEmoticonBuilder.Emoticon forground;
  }
  ...
  function getSvgDefs(SvgDescriptor memory svg) public pure returns (string memory) {
    ...
    defs = string(abi.encodePacked(defs, SvgEmoticonBuilder.getSvgDefs(svg.seed, svg.forground)));
  ...
  function getSvgStyles(SvgDescriptor memory svg) public pure returns (string memory) {
    ...
    styles = string(abi.encodePacked(styles, SvgEmoticonBuilder.getSvgStyles(svg.forground)));
  ...
  function getSvgContent(SvgDescriptor memory svg) public pure returns (string memory) {
    return SvgEmoticonBuilder.getSvgContent(svg.forground);
  }
